...
前几节学习MySQL使用的都是数据库中原本存在或是之前做实验时程序自己所创建的表，现在该学习如何自己创建一张表了，因为需求并不相同所以需要结合实际情况来创建表

比如目前我正要为网站添加一个注册的功能，这时我就需要创建一张用来存储用户账户和密码的表，这个需求是比较简单的，账户列的数据类型自然是字符串，而密码列的数据类型因为要符合一定的安全性所以不能够是单纯的整型，所以也设置为字符串

前期的表设计好之后就到了实际创建表的时候了，有两种创建表的方式，一是使用一些能够交互式创建表的工具，二便是使用sql语句创建，但实际上工具创建表使用的也是sql语句，只是这是由工具程序自动生成并执行的sql语句

创建friends表使用的是**CREATE TABLE**语句
...
![1623311483516](1623311483516.png)

对该语句做几点说明

1. 表的主键可以在创建表时使用**PRIMARY KEY**关键字指定，也可以创建表之后再定义，需要注意的主键必需是唯一的，如果主键使用的是单个列，则单个列中的值是唯一的，如果主键使用的是多个列，那么多个列的组合值必须是唯一的并且主键中只能使用不允许NULL值的列
2. 创建新表时，CREATE TABLE后给出新表的名称，该表名必需不存在，否则将出错，如果要防止意外覆盖现有的表，应该手工删除该表后再重建它，如果仅想在一个表不存在时创建它，应该再表名后跟**IF NOT EXISTS**语句
3. 对三列均使用了**NOT NULL**，这将会阻止插入没有值的列，如果试图插入没有值的列，将会插入失败并返回错误
4. 对主键id列用了**AUTO_INCREMENT**，每个表只允许一个AUTO_INCREMENT列，而且它必须被索引，比如让它成为主键，使用函数**last_insert_id()**可以返回最后一个AUTO_INCREMENT值，为一个列设置了AUTO_INCREMENT并不意味着不能再为它插入值，可以使用**INSERT**语句指定一个还没使用值，该值将替代自动生成的值，后续的增量将开始使用该手工值 

关于创建表暂时学到这里，今天的重点在于搭建SQL注入的漏洞环境 本节对应21章
